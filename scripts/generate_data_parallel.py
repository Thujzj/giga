...
def main(args, rank):
    GRASPS_PER_SCENE = args.grasps_per_scene    #120
    np.random.seed()
    seed = np.random.randint(0, 1000) + rank
    np.random.seed(seed)
    sim = ClutterRemovalSim(args.scene, args.object_set, gui=args.sim_gui) #pile pile/train  true
    
    finger_depth = sim.gripper.finger_depth
    grasps_per_worker = args.num_grasps // args.num_proc     #4000000/40
