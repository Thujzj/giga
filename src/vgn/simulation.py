...
class ClutterRemovalSim(object):
    ...
    def reset(self, object_count):
        ...
        table_height = self.gripper.finger_depth   #0.05
        self.place_table(table_height)              #放置平板

        if self.scene == "pile":
            self.generate_pile_scene(object_count, table_height)
        elif self.scene == "packed":
            self.generate_packed_scene(object_count, table_height)#object_count：应该是场景中的物体数量
        else:
    ...
    def place_table(self, height):
        ...
        pose = Transform(Rotation.identity(), [0.15, 0.15, height])  #桌子的位置：[0.15, 0.15, 0.05]
        self.world.load_urdf(urdf, pose, scale=0.6)

        # define valid volume for sampling grasps
        lx, ux = 0.02, self.size - 0.02      #0.02,0.03
    ...
    def generate_packed_scene(self, object_count, table_height):
        ...
        while self.num_objects < object_count and attempts < max_attempts:
            ...
            # print("Transform(rotation, np.r_[x, y, z])::\n",Transform(rotation, np.r_[x, y, z]).as_matrix())
            self.world.step()
    ...
    def acquire_tsdf(self, n, N=None, resolution=40):
        ...
        for extrinsic in extrinsics:
            ...
            # print("depthimage", depth_img)
            k = 0
            while 1:
                if os.path.exists("scripts/image/depthimage{}.png".format(k)):
                    k += 1
                    continue
                break
            plt.imsave("scripts/image/depthimage{}.png".format(k), depth_img, cmap='gray')
            plt.imsave("scripts/image/rgbimage{}.png".format(k), RGB)
